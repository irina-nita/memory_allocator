...
/* Node size including payload, header and footer. Fit for free & allocated (on heap) blocks. */
#define HEAP_INC(x) (ALLOC_SIZE(x) + sizeof(uint64_t) + FOOTER_SIZE)

/** Node size including payload and header.
 * Fit for free & allocated (on memory mapped region) blocks.
 */
...
typedef struct header Header; /* Header for free blocks */
typedef uint64_t Footer; /* Footer for heap blocks */
...
/**
 * @brief            Creates a new free block with header and footer
 * @param size       Size of the block
 * @return           Pointer to the header of the free block
*/
...
/**
 * @brief            Finds the first free block of given minimum size in bucket
 * @param bucket_idx Index of the bucket to search in
 * @param size       Minimum size for the block
 * @return           Pointer to the header of the free block
*/
Header* first_fit(uint32_t bucket_idx, uint64_t size) __attribute__ ((visibility ("internal")));

/**
 * @brief            Finds the first free block of given minimum size
 * @param size       Minimum size for the block
 * @return           Pointer to the header of the free block
*/
...
/**
 * @brief            Gets a free block of given minimum size by either finding it in the free list
...
/**
 * @brief            Creates a new free block and adds it to bucket
 * @param size       Size required for the free block.
 * @return           Pointer to the header of the free block              
*/
Header* new_free_blk(uint64_t size) __attribute__ ((visibility ("internal")));

/**
 * @brief            Check if the previous block in terms of address is free or allocated.
 * @param free_blk   Current free block
 * @return           8-bit integer as 0 or 1
*/
uint8_t check_prev_free(Header *free_blk) __attribute__ ((visibility ("internal")));

/**
 * @brief            Check if the next block in terms of address is free or allocated.
 * @param free_blk   Current free block
 * @return           8-bit integer as 0 or 1
*/
...
/**
 * @brief            Mark a block as allocated by changing the header and footer
 * @param free_blk   Current free block
 * @return           Void
*/
...
/**
 * @brief            Mark a block as free by changing the header and footer
 * @param alloc_blk  Current allocated block
 * @return           Void
*/
void mark_dealloc(Header* alloc_blk) __attribute__ ((visibility ("internal")));

/**
 * @brief            Split a free block in an allocated block
 *                   of given size and a free block of the remaining size
 * @param free_blk   Free block to split
...
/**
 * @brief            Coalesce the free block with previous/next (in terms of address) free block
 * @param free_blk   Free block
 * @return           Pointer to the header of the coalesced block
*/
Header* coalesce(Header *free_blk) __attribute__ ((visibility ("internal")));

/**
 * @brief            Coalesce the free block with next (in terms of address) free block
 * @param free_blk   Free block
 * @return           Pointer to the header of the coalesced block
*/
...
/**
 * @brief            Coalesce the free block with previous (in terms of address) free block
 * @param free_blk   Free block
 * @return           Pointer to the header of the coalesced block
*/
